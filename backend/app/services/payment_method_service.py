...
class PaymentMethodService:
    ...
    async def create_payment_method(self, user_id: int, payload: PaymentMethodCreateRequest) -> PaymentMethod:
        payment_method = PaymentMethod(
            user_id=user_id,
            type=payload.type,
    ...
    async def delete_payment_method(self, user_id: int, payment_method_id: int) -> None:
        payment_method = await self._get_owned(user_id, payment_method_id)
        await self._block_if_has_transactions(payment_method_id)
        await self.payment_method_repository.delete(payment_method)
    ...
    async def _block_if_has_transactions(self, payment_method_id: int) -> None:
        if await self.payment_method_repository.has_transactions(payment_method_id):
            raise AppException(
                status_code=409,
                error_code="PAYMENT_METHOD_HAS_TRANSACTIONS",
                message="This payment method has transactions linked to it, so it can't be archived or deleted.",
            )